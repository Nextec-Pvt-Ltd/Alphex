...
class PositionwiseFF(nn.Module):
    def __init__(self, d_model, d_inner, dropout, pre_lnorm=False):
        ...
        self.CoreNet = nn.Sequential(
            nn.Linear(d_model, d_inner), GEGLU(),
            nn.Dropout(dropout),
...
class Alphex(nn.Module):
    def __init__(self, config):
        ...
        d_embed = config.d_model if config.d_embed is None else config.d_embed
        ...
        # use adaptive softmax (including standard softmax)
            ...
            if config.tie_projs:
                for i, config.tie_proj in enumerate(tie_projs):
                    if config.tie_proj and config.div_val == 1 and config.d_model != d_embed:
                        self.crit.out_projs[i] = self.word_emb.emb_projs[0]
                    elif config.tie_proj and div_val != 1:
...
if __name__ == '__main__':
    ...
    for div_val in [1, 2]:
        for d_embed in [200, 100]:
            model = Alphex(args.n_token, args.n_layer, args.n_head,
